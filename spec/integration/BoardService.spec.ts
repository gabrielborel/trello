...
describe("Board service", () => {
  test("should create a board", async () => {
    ...
    const boardId = await boardService.saveBoard("Project Two");
    const board = await boardService.getBoard(boardId);
    expect(board.name).toBe("Project Two");
  ...
  test("should delete a board", async () => {
    ...
    const boardId = await boardService.saveBoard("Project Three");
    await boardService.deleteBoard(boardId);
    await expect(() => boardService.getBoard(boardId)).rejects.toThrow();
    await conn.close();
  });
});
